import 'package:flutter/material.dart';

class AppColors {
  static const blue_ocean = Color(0xff3669C9);
  static const dark_blue = Color(0xff0C1A30);
  static const orange_fresh = Color(0xffFDA429);
  static const rating_color = Color(0xffFFC120);
  static const deep_orange = Color(0xffFFF6E4);
  static const navy_black = Color(0xff13181F);
  static const white = Color(0xffFFFFFF);
  static const black = Color(0xff000000);
  static const earth_green = Color(0xff3A9B7A);
  static const green100 = Color(0xffE4F3EA);
  static const red_velvet = Color(0xffC93545);
  static const dark_red = Color(0xffFE3A30);
  static const soft_red = Color(0xffFFECE8);
  static const grey = Color(0xffC4C5C4);
  static const dark_grey = Color(0xffABABAB);
  static const half_grey = Color(0xff838589);
  static const grey93 = Color(0xffEDEDED);
  static const alabaster = Color(0xffFAFAFA);
  static const off_blue = Color(0xffEFF5FB);
  static const off_green = Color(0xffEEFAF6);
  static const off_red = Color(0xffFCECEF);
  static const soft_indigo = Color(0xffF1EDFC);
}
